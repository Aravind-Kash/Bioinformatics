...
{
    setRawPwm(1000, 0, 0, 900);
    //dist=30;
   // char buf[128];
    while(WTIMER0_TAV_R < (46*dist)/21 && WTIMER2_TAV_R <(46*dist)/21);
    {
...
void right(uint8_t angle)
{
    ...
    angle=6.75;
    //char buf[128];
    while(WTIMER2_TAV_R < (46*angle)/23);
    {
        //sprintf(buf,"%u %u\n",WTIMER0_TAV_R, WTIMER2_TAV_R);
        //putsUart0(buf);
    }
...
void rotatecw(uint8_t dist)
{
    setRawPwm(1000, 0, 975, 0);
    //number=2;
    while(WTIMER2_TAV_R < (46*2*dist) && WTIMER0_TAV_R < (46*2*dist));
    setRawPwm(0,0,0,0);

}

void rotateccw(uint8_t dist)
{
    setRawPwm(0, 1000, 0, 1000);
    //number=2;
    while(WTIMER2_TAV_R < (46*2*dist) && WTIMER0_TAV_R < (46*2*dist));
    setRawPwm(0,0,0,0);
